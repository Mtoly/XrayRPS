...
for required in 'User=root' 'Group=root' 'NoNewPrivileges=true' 'PrivateTmp=true' 'PrivateDevices=true' 'ProtectHome=true' 'ProtectSystem=strict' 'ProtectKernelTunables=true' 'ProtectKernelModules=true' 'ProtectControlGroups=true' 'RestrictSUIDSGID=true' 'LimitCORE=0'; do
  grep -qF -- "$required" "$service" || fail "missing $required"
done
if grep -qF 'CAP_NET_BIND_SERVICE' "$service"; then
    fail 'root compatibility must not add CAP_NET_BIND_SERVICE'
fi
if grep -qE '^LimitCORE=infinity$' "$service"; then fail 'service still allows unlimited core dumps'; fi
